...
{
    public class MsgEliteGuildWar3rd
    {
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 6629));
        }

        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 19 && Now.Minute == 41 && Now.Second == 00)
                this.Start();

            if (Proces == ProcesType.Alive)
            {

                if (Now.Minute == 50 && Now.Second == 01)
                    MsgSchedules.SendSysMesage("EliteGuildWar 3rd Pole will Ended after 10 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Minute == 55 && Now.Second == 01)
                    MsgSchedules.SendSysMesage("EliteGuildWar 3rd Pole will ended after 5 Minutes.", MsgServer.MsgMessage.ChatMode.Center);
        ...
        public void Start()
        {
            ...
            MsgSchedules.SendInvitation("EliteGuildWar 3rd Pole  has begun! Would you like to join?", "Special Prize", 419, 259, 1002, 0, 120);
        }
        ...
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
                MsgSchedules.SendSysMesage(RoundOwner == null ? "EliteGuildWar 3rd Pole has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner.", MsgServer.MsgMessage.ChatMode.Center);
                ScoreList.Clear();
            }
        }
