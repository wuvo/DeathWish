...
{
    public class MsgEliteGuildWar1st
    {
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 6627));
        }

        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 19 && Now.Minute == 00 && Now.Second == 10)
                this.Start();
            ...
            if (Proces == ProcesType.Alive)
            {
                ...
                if (Now.Minute == 10 && Now.Second == 10)
                    MsgSchedules.SendSysMesage("EliteGuildWar 1st Pole will Ended after 10 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Minute == 15 && Now.Second == 10)
                    MsgSchedules.SendSysMesage("EliteGuildWar 1st Pole will ended after 5 Minutes.", MsgServer.MsgMessage.ChatMode.Center);
        ...
        public void Start()
        {
            ...
            MsgSchedules.SendInvitation("EliteGuildWar 1st Pole  has begun! Would you like to join?", "Special Prize", 419, 259, 1002, 0, 120);
        }
        ...
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
                MsgSchedules.SendSysMesage(RoundOwner == null ? "EliteGuildWar 1st Pole has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner.", MsgServer.MsgMessage.ChatMode.Center);
                ScoreList.Clear();
            }
        }
