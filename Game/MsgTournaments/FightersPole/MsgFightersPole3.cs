...
namespace DeathWish.Game.MsgTournaments
{
    public class MsgFightersPole3
    {
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 2279));
        }
        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 14 && Now.Minute == 31 && Now.Second <= 6)
                this.Start();
            if (Proces == ProcesType.Alive)
            {
                if (Now.Hour == 14 && Now.Minute == 40 && Now.Second <= 2)
                 //   MsgSchedules.SendSysMesage("FightersWar Pole[3] will ended after 5 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 43 && Now.Second <= 2)
                //    MsgSchedules.SendSysMesage("FightersWar Pole[3] will ended after 2 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 45 && Now.Second > 2)
                    this.End();
            }
        }
        public void Start()
      ...
      //      MsgSchedules.SendSysMesage("FightersWar Pole[3] war has began", MsgServer.MsgMessage.ChatMode.Center);
          //  MsgSchedules.SendInvitation("FightersWar Pole[3] has begun! Would you like to join?", "CPs", 336, 137, 1002, 0, 60);
        }
        ...
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
            //    MsgSchedules.SendSysMesage(RoundOwner == null ? "FightersWar Pole[3] has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner of FightersWar Pole[3].", MsgServer.MsgMessage.ChatMode.Center);
                ScoreList.Clear();
            }
        }
