...
namespace DeathWish.Game.MsgTournaments
{
    public class MsgFightersPole1
    {
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 2277));
        }
        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 14 && Now.Minute == 00 && Now.Second <= 2)
                this.Start();
            if (Proces == ProcesType.Alive)
            {
                if (Now.Hour == 14 && Now.Minute == 10 && Now.Second <= 2)
                 //   MsgSchedules.SendSysMesage("FightersWar Pole[1] will ended after 5 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 13 && Now.Second <= 2)
                //    MsgSchedules.SendSysMesage("FightersWar Pole[1] will ended after 2 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 15 && Now.Second > 2)
                    this.End();
            }
        }
        public void Start()
        {
           ...
           // MsgSchedules.SendSysMesage("FightersWar Pole[1] war has began", MsgServer.MsgMessage.ChatMode.Center);
         //   MsgSchedules.SendInvitation("FightersWar Pole[1] has begun! Would you like to join?", "CPs", 336, 137, 1002, 0, 60);
        }
       ...
       //     MsgSchedules.SendSysMesage("The [ Pole[1] ] Round Ownered by guild " + RoundOwner.GuildName + ".", MsgServer.MsgMessage.ChatMode.Center);
            Reset(stream);
        }
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
             //   MsgSchedules.SendSysMesage(RoundOwner == null ? "FightersWar Pole[1] has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner of FightersWar Pole[1].", MsgServer.MsgMessage.ChatMode.Center);
                ScoreList.Clear();
            }
        }
