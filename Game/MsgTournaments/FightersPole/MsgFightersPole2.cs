...
namespace DeathWish.Game.MsgTournaments
{
    public class MsgFightersPole2
    {
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 2278));
        }
        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 14 && Now.Minute == 16 && Now.Second <= 4)
                this.Start();
            if (Proces == ProcesType.Alive)
            {
                if (Now.Hour == 14 && Now.Minute == 25 && Now.Second <= 2)
               //     MsgSchedules.SendSysMesage("FightersWar Pole[2] will ended after 5 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 28 && Now.Second <= 2)
               //     MsgSchedules.SendSysMesage("FightersWar Pole[2] will ended after 2 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Hour == 14 && Now.Minute == 30 && Now.Second > 2)
                    this.End();
            }
        }
        public void Start()
       ...
       //     MsgSchedules.SendSysMesage("FightersWar Pole[2] war has began", MsgServer.MsgMessage.ChatMode.Center);
         //   MsgSchedules.SendInvitation("FightersWar Pole[2] has begun! Would you like to join?", "CPs", 336, 137, 1002, 0, 60);
        }
        ...
        public void FinishRound(ServerSockets.Packet stream)
        {
          ...
          //  MsgSchedules.SendSysMesage("The [ Pole[2] ] Round Ownered by guild " + RoundOwner.GuildName + ".", MsgServer.MsgMessage.ChatMode.Center);
            Reset(stream);
        }
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
             //   MsgSchedules.SendSysMesage(RoundOwner == null ? "FightersWar Pole[2] has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner of FightersWar Pole[2].", MsgServer.MsgMessage.ChatMode.Center);
                ScoreList.Clear();
            }
        }
