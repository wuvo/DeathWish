                   ...
                   case (ushort)Role.Flags.SpellID.StraightFist:
                       {
                           ...
                           Algoritms.InLineAlgorithm Line = new Algoritms.InLineAlgorithm(user.Player.X, Attack.X, user.Player.Y, Attack.Y, user.Map, 5, 0, ClientSpell.ID);

                           byte LineRange = (byte)((ClientSpell.UseSpellSoul > 0) ? 1 : 0);
                           ...
                           foreach (Role.IMapObj target in user.Player.View.Roles(Role.MapObjectType.Monster))
                           {
                               ...
                               if ((attacked.Family.Settings & MsgMonster.MonsterSettings.Guard) == MsgMonster.MonsterSettings.Guard)
                                   continue;
                               if (Role.Core.GetDistance(user.Player.X, user.Player.Y, attacked.X, attacked.Y) < 5)
                               {
                           ...
                           foreach (Role.IMapObj targer in user.Player.View.Roles(Role.MapObjectType.Player))
                           {
                               ...
                               if (Role.Core.GetDistance(user.Player.X, user.Player.Y, targer.X, targer.Y) < 5)
                               {
                                   if (Line.InLine(attacked.X, attacked.Y, LineRange))
                                   {
