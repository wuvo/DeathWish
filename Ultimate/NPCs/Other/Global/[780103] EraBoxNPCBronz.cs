...
{
    public class NPC_780103 : NPCBase
    {
        public NPC_780103(Main.GameClient _client)
            : base(_client)
        {
            ID = 780103;
            Face = 1;
        ...
        public override void Run(Main.GameClient GC, byte[] _data, ushort _linkback)
        {
            ...
            switch (_linkback)
            {
                case 0:
                    {
                        AddText("Do you want to open EraBox[Bronze] with you Key ?");
                ...
                case 1:
                    {
                        if (GC.MyChar.InventoryContains(710850, 1) && GC.MyChar.Inventory.Count <= 36)
                        {
                            int j = 1;
                            ...
                            GC.MyChar.RemoveItem(GC.MyChar.NextItem(721955));//EraBronzeBox
                            GC.MyChar.RemoveItem(GC.MyChar.NextItem(710850));//key
                            ...
                            for (int l = 0; l < j; l++)
                            {
                                ...
                                {
                                    Luck.ID = 1088000;
                                    World.SendMsgToAll("LUCKY", GC.MyChar.Name + " has opened an EraBox[Bronze] and received a DragonBall!", 2005, 0);
                                }
                                else if (MyMath.ChanceSuccess(8))
                                {
                                    Luck.ID = 720028;
                                    World.SendMsgToAll("LUCKY", GC.MyChar.Name + " has opened an EraBox[Bronze] and received a DBScroll!", 2005, 0);
                                }
                            ...
                            if (MyMath.ChanceSuccess(100))
                            {
                                ...
                                int Type = GC.MyChar.Rnd.Next(0, 120);
                                uint Part = 0;
                                if (Type < 10) Part = 111;
                                ...
                                if (ItemID != 0)
                                {
                                    ...
                                    else if (ItemIDManipulation.Digit(I2.ID, 1) == 1 || ItemIDManipulation.Digit(I2.ID, 1) == 2 || ItemIDManipulation.Digit(I2.ID, 1) == 3 || ItemIDManipulation.Digit(I2.ID, 1) == 6 || ItemIDManipulation.Digit(I2.ID, 1) == 8)
                                    {
                                        ...
                                        {
                                            I2.Soc1 = Item.Gem.EmptySocket;
                                            World.SendMsgToAll("LUCKY", GC.MyChar.Name + " has opened an EraBox[Bronze] and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                            World.DebugAdd += GC.MyChar.Name + " obtained socket " + I2.DBInfo.Name + " from an EraBox[Bronze]! \r\n";
                                        }
                                    }
                            ...
                            GC.MyChar.MyClient.LocalMessage(2005, "You have successfuly opened the EraBox[Bronze]! Check your inventory!");
                        }
                        else
                        {
                            GC.MyChar.MyClient.LocalMessage(2005, "Unable to open EraBox[Bronze] . Either you don't have EraKey or you don't have at least 3 spaces in your inventory!");
                        }
                        break;
                    }
            }
