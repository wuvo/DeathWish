...
{
    public class NPC_3014 : NPCBase
    {
        public NPC_3014(Main.GameClient _client)
            : base(_client)
        {
            ID = 3014;
            Face = 7;
        ...
        public override void Run(Main.GameClient GC, byte[] _data, ushort _linkback)
        {
            ...
            switch (_linkback)
            {
                case 0:
                    {
                        ...
                        AddOption("Join City War BirdIsland", 2);
                ...
                case 2:
                    {
                        if (Features.CityWarBi.War)
                        {
                            ...
                            if (x == 1)
                                GC.MyChar.Teleport(8507, 81, 112);
                            else if (x == 2)
                                GC.MyChar.Teleport(8507, 109, 142);
                            else if (x == 3)
                                GC.MyChar.Teleport(8507, 134, 111);
                            else if (x == 4)
                                GC.MyChar.Teleport(8507, 109, 76);
                            else
                                GC.MyChar.Teleport(8507, 108, 58);
                        }
                        else
                        {
                ...
                case 3:
                    {
                        if (Features.CityWarBi.LastWinner == GC.MyChar.MyGuild && Features.CityWarBi.BiPrize == true && GC.MyChar.GuildRank == Features.GuildRank.GuildLeader)
                        {
                            if (GC.MyChar.Inventory.Count <= 39)
                            {
                                Program.WriteCmds(GC.MyChar.Name + " has got CityWarBi prize");
                                GC.MyChar.Silvers += 50000000;
                                //GC.MyChar.AddItem(700013);
                                //GC.MyChar.AddItem(700003);
                                //GC.MyChar.AddItem(700073);
                                AddText("Congratulations! You have received 50,000,000 silvers for winning the CityWarBi GuildWar!");
                                Game.World.SendMsgToAll("SYSTEM", GC.MyChar.Name + " claimed 50,000,000 silvers for winning the CityWarBi GuildWar!", 2011, 0);
                                Features.CityWarBi.BiPrize = false;
                                AddOption("Thanks.", 255);
                                break;
                            }
                            else
                        ...
                        else if (Features.CityWarBi.LastWinner == GC.MyChar.MyGuild && Features.CityWarBi.BiPrize == true)
                        {
                        ...
                        {
                            AddText("You have not won the CityWarBi or the prize has been already given.");
                            AddOption("I see.", 255);
                            break;
                        }
                    }
