...
{
    public class Item_721080 : IItem
    {
        public override void Run(Character C, Item I)
        {
            if (C.Inventory.Count <= 36)
            {
                ...
                for (int l = 0; l < j; l++)
                {
                    ...
                    {
                        Luck.ID = 1088000;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a MoonBox and received a DragonBall!", 2005, 0);
                    }
                ...
                if (MyMath.ChanceSuccess(90))
                {
                    ...
                    if (ItemID != 0)
                    {
                        ...
                        else if (ItemIDManipulation.Digit(I2.ID, 1) == 1 || ItemIDManipulation.Digit(I2.ID, 1) == 2 || ItemIDManipulation.Digit(I2.ID, 1) == 3 || ItemIDManipulation.Digit(I2.ID, 1) == 6 || ItemIDManipulation.Digit(I2.ID, 1) == 8)
                        {
                            ...
                            {
                                I2.Soc1 = Item.Gem.EmptySocket;
                                World.SendMsgToAll("LUCKY", C.Name + " has opened a MoonBox and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                World.DebugAdd += C.Name + " obtained socket " + I2.DBInfo.Name + " from a MoonBox! \r\n";
                            }
                        }
                ...
                C.MyClient.LocalMessage(2005, "You have successfuly opened the MoonBox! Check your inventory!");
            }
            else
            {
