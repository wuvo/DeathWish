...
{
    public class Item_721544 : IItem
    {
            ...
            if (C.Inventory.Count <= 37)
            {
                ...
                for (int l = 0; l < j; l++)
                {
                    ...
                    if (MyMath.ChanceSuccess(9))
                        Luck.ID = 720027;
                    else if (MyMath.ChanceSuccess(4.5))
                    {
                        Luck.ID = 1088000;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a CloudBox and received a DragonBall!", 2005, 0);
                    }
                    else if (MyMath.ChanceSuccess(1))
                    {
                        Luck.ID = 1088000;
                        C.AddItem(1088000);
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a CloudBox and received 2 DragonBalls!", 2005, 0);
                    }
                    else
                        Luck.ID = 1088001;
                    C.AddItem(Luck);
                ...
                if (MyMath.ChanceSuccess(90))
                {
                    ...
                    if (ItemID != 0)
                    {
                        ...
                        else if (ItemIDManipulation.Digit(I2.ID, 1) == 1 || ItemIDManipulation.Digit(I2.ID, 1) == 2 || ItemIDManipulation.Digit(I2.ID, 1) == 3 || ItemIDManipulation.Digit(I2.ID, 1) == 6 || ItemIDManipulation.Digit(I2.ID, 1) == 8)
                        {
                            ...
                            {
                                I2.Soc1 = Item.Gem.EmptySocket;
                                World.SendMsgToAll("LUCKY", C.Name + " has opened a CloudBox and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                World.DebugAdd += C.Name + " obtained socket " + I2.DBInfo.Name + " from a CloudBox! \r\n";
                            }
                        }
                ...
                C.MyClient.LocalMessage(2005, "You have successfuly opened the CloudBox! Check your inventory!");
            }
            else
            {
                C.MyClient.LocalMessage(2005, "You need at least 3 spaces in your inventory!");
            }
        }
    }
}