...
{
    public class Item_721543 : IItem
    {
            ...
            if (C.Inventory.Count <= 37)
            {
                ...
                for (int l = 0; l < j; l++)
                {
                    ...
                    if (MyMath.ChanceSuccess(8))
                        Luck.ID = 720027;
                    else if (MyMath.ChanceSuccess(4))
                    {
                        Luck.ID = 1088000;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a StarBox and received a DragonBall!", 2005, 0);
                    }
                    else if (MyMath.ChanceSuccess(0.8))
                    {
                        Luck.ID = 1088000;
                        C.AddItem(1088000);
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a StarBox and received 2 DragonBalls!", 2005, 0);
                    }
                    else
                        Luck.ID = 1088001;
                    C.AddItem(Luck);
                ...
                if (MyMath.ChanceSuccess(90))
                {
                    ...
                    if (ItemID != 0)
                    {
                        ...
                        {
                            if (MyMath.ChanceSuccess(1.7))
                            {
                                I2.Soc1 = Item.Gem.EmptySocket;
                                World.SendMsgToAll("LUCKY", C.Name + " has opened a StarBox and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                World.DebugAdd += C.Name + " obtained socket " + I2.DBInfo.Name + " from a StarBox! \r\n";
                            }
                        }
                ...
                C.MyClient.LocalMessage(2005, "You have successfuly opened the StarBox! Check your inventory!");
            }
            else
            {
