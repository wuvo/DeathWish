...
{
    public class Item_721541 : IItem
    {
        public override void Run(Character C, Item I)
        {
            if (C.Inventory.Count <= 37)
            {
                ...
                for (int l = 0; l < j; l++)
                {
                    ...
                    {
                        Luck.ID = 1088000;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a SunBox and received a DragonBall!", 2005, 0);
                    }
                    else if (MyMath.ChanceSuccess(0.5))
                    {
                        Luck.ID = 1088000;
                        C.AddItem(1088000);
                        World.SendMsgToAll("LUCKY", C.Name + " has opened a SunBox and received 2 DragonBalls!", 2005, 0);
                    }
                ...
                if (MyMath.ChanceSuccess(90))
                {
                    ...
                    if (ItemID != 0)
                    {
                        ...
                        else if (ItemIDManipulation.Digit(I2.ID, 1) == 1 || ItemIDManipulation.Digit(I2.ID, 1) == 2 || ItemIDManipulation.Digit(I2.ID, 1) == 3 || ItemIDManipulation.Digit(I2.ID, 1) == 6 || ItemIDManipulation.Digit(I2.ID, 1) == 8)
                        {
                            ...
                            {
                                I2.Soc1 = Item.Gem.EmptySocket;
                                World.SendMsgToAll("LUCKY", C.Name + " has opened a SunBox and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                World.DebugAdd += C.Name + " obtained socket " + I2.DBInfo.Name + " from a SunBox! \r\n";
                            }
                        }
                ...
                C.MyClient.LocalMessage(2005, "You have successfuly opened the SunBox! Check your inventory!");
            }
            else
