...
{
    public class Item_721540 : IItem
    {
        public override void Run(Character C, Item I)
        {
            if (C.Inventory.Count <= 36)
            {
                ...
                for (int l = 0; l < j; l++)
                {
                    ...
                    {
                        Luck.ID = 1088000;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened an AncestorBox and received a DragonBall!", 2005, 0);
                    }
                    else if (MyMath.ChanceSuccess(5))
                    {
                        Luck.ID = 720028;
                        World.SendMsgToAll("LUCKY", C.Name + " has opened an AncestorBox and received a DBScroll!", 2005, 0);
                    }
                ...
                if (MyMath.ChanceSuccess(100))
                {
                    ...
                    if (ItemID != 0)
                    {
                        ...
                        else if (ItemIDManipulation.Digit(I2.ID, 1) == 1 || ItemIDManipulation.Digit(I2.ID, 1) == 2 || ItemIDManipulation.Digit(I2.ID, 1) == 3 || ItemIDManipulation.Digit(I2.ID, 1) == 6 || ItemIDManipulation.Digit(I2.ID, 1) == 8)
                        {
                            ...
                            {
                                I2.Soc1 = Item.Gem.EmptySocket;
                                World.SendMsgToAll("LUCKY", C.Name + " has opened an AncestorBox and received a socket " + I2.DBInfo.Name + "!", 2011, 0);
                                World.DebugAdd += C.Name + " obtained socket " + I2.DBInfo.Name + " from an AncestorBox! \r\n";
                            }
                        }
                ...
                C.MyClient.LocalMessage(2005, "You have successfuly opened the AncestorBox! Check your inventory!");
            }
            else
            {
