...
namespace Ultimate.Features
{
    public class TopSpouse
    {
        ...
        private static readonly List<Team> SpouseTeams = new List<Team>();
        private static readonly List<Game.Character> TeamMembers = new List<Game.Character>();
        private static readonly List<Team> RemoveTeams = new List<Team>();
        ...
        public void StartTournament()
        {
            ...
            foreach (Team T in SpouseTeams)
                foreach (Character C in T.Members)
                    World.Action(C, (Packets.StringPacket(C.EntityID, StringType.Effect, "downnumber5")).Get);
            Thread.Sleep(1000);
            foreach (Team T in SpouseTeams)
                foreach (Character C in T.Members)
                    World.Action(C, (Packets.StringPacket(C.EntityID, StringType.Effect, "downnumber4")).Get);
            Thread.Sleep(1000);
            foreach (Team T in SpouseTeams)
                foreach (Character C in T.Members)
                    World.Action(C, (Packets.StringPacket(C.EntityID, StringType.Effect, "downnumber3")).Get);
            Thread.Sleep(1000);
            foreach (Team T in SpouseTeams)
                foreach (Character C in T.Members)
                    World.Action(C, (Packets.StringPacket(C.EntityID, StringType.Effect, "downnumber2")).Get);
            Thread.Sleep(1000);
            foreach (Team T in SpouseTeams)
                foreach (Character C in T.Members)
                    World.Action(C, (Packets.StringPacket(C.EntityID, StringType.Effect, "downnumber1")).Get);
            Removeprotection();
        ...
        public void TeleportPlayersToMap()
        {
            for (int i = 0; i < SignupTeams.Count + SpouseTeams.Count; i++)
            {
                ...
                var X = (ushort)Program.Rnd.Next(108, 141);
                var Y = (ushort)Program.Rnd.Next(126, 159);
                foreach (Game.Character C in ((Team)(SignupTeams[TakeTeam])).Members)
                {
                    if (!C.Alive)
                    {
                                ...
                                foreach (Game.Character C in T.Members)
                                {
                                    if (C.MyClient.Soc.Connected)
                                    {
                                        if (T.Leader != null && C.MyTeam.Leader.EntityID == T.Leader.EntityID)
                                        {
                                            ...
                                            if (C.MyClient.MyChar.Spouse != null && C.MyClient.MyChar.MyTeam.Members.Contains(Love) || T.Leader.MyClient.PM)
                                                continue;
