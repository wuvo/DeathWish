...
{
    public class PoleWarTC
    {
        public static bool signup = false;
        public static bool PoleTcPrize = false;
        ...
        public static void Init()
        {
            ...
            ThePole = new SOB() { EntityID = 6732, Type = Looks.Pole, Mesh = 1137, CurHP = 20000000, MaxHP = 20000000, Loc = new Location() { Map = 1002, X = 430, Y = 256 }, War = false, LastWinner = LastWinner };
            ThePole.AddSOB();
        ...
        public static void StartWar()
        {
            ...
            loadstats();
            //World.SendMsgToAll("SYSTEM", "PoleWarTC has begun! Talk to ObscureWarrior in TwinCity to join!", 2011, 0);
            War = true;
        }
        ...
        public static void EndWarForGood()
        {
            ...
            Cmd2.Update("guildwars").Set("winner", Features.PoleWarTC.LastWinner.GuildName).Where("id", 9).Execute();
            foreach (Character C in World.H_Chars.Values)
                if (C.Loc.Map == 1002)
                    ChangePKMode(C, PKMode.Capture);
            if (LastWinner != null)
            {
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the TC Pole War! Congratulations!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the TC Pole War! Congratulations!", 2000, 0);
            }
            else
                World.SendMsgToAll("SYSTEM", "The TC Pole War has ended and there were no winners!", 2000, 0);
        }
        ...
        public static void PoleTakedown()
        {
            ...
            if (Highest.TheGuild != null)
            {
                ...
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " TC Pole War have won!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " TC Pole War have won!", 2000, 0);
            }
