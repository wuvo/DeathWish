...
{
    public class PoleWarPC
    {
        public static bool signup = false;
        public static bool PolePcPrize = false;
        ...
        public static void Init()
        {
            ...
            ThePole = new SOB() { EntityID = 6733, Type = Looks.Pole, Mesh = 1137, CurHP = 20000000, MaxHP = 20000000, Loc = new Location() { Map = 1011, X = 207, Y = 337 }, War = false, LastWinner = LastWinner };
            ThePole.AddSOB();
        ...
        public static void StartWar()
        {
            ...
            loadstats();
            //World.SendMsgToAll("SYSTEM", "PoleWarPC has begun! Talk to ObscureWarrior in TwinCity to join!", 2011, 0);
            War = true;
        }
        ...
        public static void EndWarForGood()
        {
            ...
            Cmd2.Update("guildwars").Set("winner", Features.PoleWarPC.LastWinner.GuildName).Where("id", 9).Execute();
            foreach (Character C in World.H_Chars.Values)
                if (C.Loc.Map == 1011)
                    ChangePKMode(C, PKMode.Capture);
            if (LastWinner != null)
            {
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the PC Pole War! Congratulations!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the PC Pole War! Congratulations!", 2000, 0);
            }
            else
                World.SendMsgToAll("SYSTEM", "The PC Pole War has ended and there were no winners!", 2000, 0);
        }
        ...
        public static void PoleTakedown()
        {
            ...
            if (Highest.TheGuild != null)
            {
                ...
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " PC Pole War have won!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " PC Pole War have won!", 2000, 0);
            }
