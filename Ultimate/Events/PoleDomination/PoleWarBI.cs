...
{
    public class PoleWarBI
    {
        public static bool signup = false;
        public static bool PoleBiPrize = false;
        ...
        public static void Init()
        {
            ...
            ThePole = new SOB() { EntityID = 6736, Type = Looks.Pole, Mesh = 1137, CurHP = 20000000, MaxHP = 20000000, Loc = new Location() { Map = 1015, X = 816, Y = 534 }, War = false, LastWinner = LastWinner };
            ThePole.AddSOB();
        ...
        public static void StartWar()
        {
            ...
            loadstats();
            //World.SendMsgToAll("SYSTEM", "PoleWarBI has begun! Talk to ObscureWarrior in TwinCity to join!", 2011, 0);
            War = true;
        }
        ...
        public static void EndWarForGood()
        {
            ...
            Cmd2.Update("guildwars").Set("winner", Features.PoleWarBI.LastWinner.GuildName).Where("id", 9).Execute();
            foreach (Character C in World.H_Chars.Values)
                if (C.Loc.Map == 1015)
                    ChangePKMode(C, PKMode.Capture);
            if (LastWinner != null)
            {
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the BI Pole War! Congratulations!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " have won the BI Pole War! Congratulations!", 2000, 0);
            }
            else
                World.SendMsgToAll("SYSTEM", "The BI Pole War has ended and there were no winners!", 2000, 0);
        }
        ...
        public static void PoleTakedown()
        {
            ...
            if (Highest.TheGuild != null)
            {
                ...
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " BI Pole War have won!", 2011, 0);
                World.SendMsgToAll("SYSTEM", LastWinner.GuildName + " BI Pole War have won!", 2000, 0);
            }
