...
namespace DeathWish.Role.Instance
{
    ...
    {
        public Chi.ChiAttributeType type;
        public int lvl;
        public Retreating(Chi.ChiAttributeType type, int lvl)
        {
            this.type = type;
            this.lvl = lvl;
        }
        public static List<Retreating> _ToList(Tuple<Chi.ChiAttributeType, int>[] data)
        {
            List<Retreating> _val = new List<Retreating>();
            foreach (var _data in data)
                _val.Add(new Retreating(_data.Item1, _data.Item2));
            return _val;
        }
        public static Tuple<Chi.ChiAttributeType, int>[] _ToTuple(List<Retreating> data)
        {
            Tuple<Chi.ChiAttributeType, int>[] _val = new Tuple<Chi.ChiAttributeType, int>[4];
            byte x = 0;
            foreach (var _data in data)
            {
                _val[x] = new Tuple<Chi.ChiAttributeType, int>(_data.type, _data.lvl);
                x++;
            }
            return _val;
        }
    ...
    public class Chi : IEnumerable<Chi.ChiPower>
    {
        ...
        public class ChiPower
        {
            ...
            public uint Closed;
            public int Score { get { return UnLocked ? this.Fields.Sum((x) => GetPonits(x.Item1, x.Item2)) : 0; } }
            ...
            {
                this.Fields = new Tuple<ChiAttributeType, int>[4];
                this.Type = _Type;
            }

            public void RerollGM(int Gate, ChiAttributeType PowerType, ushort Value)
            {
                lock (this.Fields)
                {
                    if (Gate < 1)
            ...
            public void Reroll(Game.MsgServer.MsgChiInfo.LockedFlags Locked, bool GMCommand = false)
            {
                lock (this.Fields)
                {
                    for (int x = 0; x < this.Fields.Length; x++)
                    {
                        ...
                        this.Fields[x] = Tuple.Create(attribute, StatsRand(attribute, Type, Name, false, GMCommand));
                    }
                }
            }
            ...
            public static int StatsRand(ChiAttributeType typ, Game.MsgServer.MsgChiInfo.ChiPowerType StageType, string Name, bool epic = false, bool GMCommand = false)
            {
                ...
                if ((byte)typ < 6)
                {
                    ...
                    if (GMCommand)
                    {
                        int gmval = Rand.Next(190, 200);
                        return (int)((uint)data.Item1 * 10000 + gmval);
                    }
            ...
            public void CreateCustomAttribut(ChiAttributeType type, int index)
            {
                ChiAttributeFlags flag = AttributeToFlag(type);
                if (Fields[index] != null)
                {
                    this.attributeFlags &= ~AttributeToFlag(Fields[index].Item1);
                }
                this.attributeFlags |= flag;

                Fields[index] = Tuple.Create(type, StatsRand(type, Type, Name, true));
            }
            public bool ContainAtribut(ChiAttributeType atribut)
            {
                var flag = AttributeToFlag(atribut);
            ...
            public void Load(string line, uint _UID, string _Name)
            {
                ...
                if (this.UnLocked)
                {
                    ...
                    for (int x = 0; x < Fields.Length; x++)
                    {
                        ...
                        this.Fields[x] = Tuple.Create(atr, role);
        ...
        public string Name = "";
        public uint UID;
        public ChiPower GetChiPowerByMode(DeathWish.Game.MsgServer.MsgChiInfo.ChiPowerType Mode)
        {
            if (Mode == MsgChiInfo.ChiPowerType.Dragon)
                return Dragon;
            if (Mode == MsgChiInfo.ChiPowerType.Phoenix)
                return Phoenix;
            if (Mode == MsgChiInfo.ChiPowerType.Turtle)
                return Turtle;
            if (Mode == MsgChiInfo.ChiPowerType.Tiger)
                return Tiger;
            return null;
        }
        ...
        public List<Retreating> DragonPower = null;
        public List<Retreating> PhoenixPower = null;
        public List<Retreating> TurtlePower = null;
        public List<Retreating> TigerPower = null;
