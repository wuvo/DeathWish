...
200, //3s line
        ...
        public unsafe uint GetRewardAmount(Client.GameClient client, ServerSockets.Packet stream)
        {
            ...
            if (win == 0)
                if (Cps)
                    client.GainExpBall(BetAmount * 3.5, true);
            return win;
        }
            ...
            for (int i = 2; i >= 0; i--)
            {
                while (true)
                {
                    ...
                    else
                    {
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(180))
                            Wheels[i] = SlotMachineItems.ExpBall;
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(210))
                            Wheels[i] = SlotMachineItems.SwordAndShield;
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(180))
                            Wheels[i] = SlotMachineItems.TwoSwords;
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(120))
                            Wheels[i] = SlotMachineItems.Sword;
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(540))
                            Wheels[i] = SlotMachineItems.Meteor;
                        Wheels[i] = (SlotMachineItems)0;
                        if (Core.Rate(600))
                            Wheels[i] = SlotMachineItems.DragonBall;
                        Wheels[i] = (SlotMachineItems)0;
