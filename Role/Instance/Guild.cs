...
namespace DeathWish.Role.Instance
{
    public class Guild
    {
        ...
        {
            const uint ConquerPoints = 50000
                , Money = 5000000;
        ...
        public Union GetUnion
        {
            ...
            {
                Union _union;
                if (Union.UnionPoll.TryGetValue(UnionID, out _union))
                    return _union;
                return null;
            }
        }
        ...
        public unsafe void AddPlayer(Role.Player player, ServerSockets.Packet stream)
        {
            if (player.MyGuild == null)
            {
                ...
                if (player.MyUnion != null)
                {
                    if (player.MyUnion.UID == this.UnionID)
                    {
            ...
            if (membru != null)
            {
                ...
                switch (rank)
                {
                    case (ushort)Role.Flags.GuildMemberRank.GuildLeader:
                        {
                            ...
                            if (union != null)
                            {
                                if (union.EmperrorUID == owner.UID)
                                {
                                    union.EmperrorUID = membru.UID;
                                    union.Emperor = membru.Name;
                                    if (membru.UnionMem != null)
                                    {
                                        union.RemoveEmperor();
                                        union.AddEmperror(membru.UnionMem);
                                    }
                                }
                            }
                    ...
                    case (ushort)Role.Flags.GuildMemberRank.DeputyLeader:
                        {
                            bool online = false;
                            if (RanksCounts[(ushort)Role.Flags.GuildMemberRank.DeputyLeader] == 50) return false;
                            RanksCounts[rank]++;
        ...
        public static bool AllowCreateGuild(string Name)
        {
            ...
            foreach (var nname in ChangeNameRecords.Values)
                if (nname == Name)
                    return false;
            foreach (var guild in GuildPoll.Values)
                if (guild.GuildName == Name)
                    return false;
            return true;
        }

        public static void RegisterChangeName(uint guildid, string name)
        {
            if (!ChangeNameRecords.ContainsKey(guildid))
                ChangeNameRecords.TryAdd(guildid, name);
            else
                ChangeNameRecords[guildid] = name;
        ...
        public static void ProcessChangeNames()
        {
            ...
            {
                Guild guild;
                if (GuildPoll.TryGetValue(record.Key, out guild))
                {
                    guild.GuildName = record.Value;
                }
            }
        }
        ...
        public void Quit(string name, bool ReceiveKick, ServerSockets.Packet stream)
        {
            ...
            if (Database.Server.GamePoll.TryGetValue(member.UID, out client))
            {
                ...
                if (member.UnionMem != null)
                {
                    var union = GetUnion;
                    if (union != null)
                        union.AddOtherMember(stream, client);
                    else
                        if (client.Player.InUnion)
                            client.Player.MyUnion.AddOtherMember(stream, client);
                }
            }
            else
            {
            ...
            if (member.UnionMem != null)
            {
                var union = GetUnion;
                if (union != null)
                    union.AddOtherMember(stream, member.UnionMem);
            }
        ...
        public override string ToString()
        {
            ...
            return writer.Add(Info.GuildID).Add(GuildName).Add(Info.LeaderName).Add(Info.SilverFund).Add(Info.ConquerPointFund)
                  .Add(Info.CreateTime).Add(Bulletin).Add((byte)(UseAdvertise ? 1 : 0)).Add(BuletinEnrole).Add(UnionID).Close();
        }
        public class Arsenal
        {
            ...
            public static byte GetArsenalPosition(uint ID)
            {
                if ((ID >= 111003 && ID <= 118309) || (ID >= 123000 && ID <= 123309) || (ID >= 141003 && ID <= 145309)
                    ...
                    return Tower;
                else if (ID >= 410003 && ID <= 617439 || Database.ItemType.IsNinjaEpicWeapon(ID) || Database.ItemType.IsTrojanEpicWeapon(ID) || Database.ItemType.IsMonkEpicWeapon(ID)
                    || Database.ItemType.IsWarriorEpicWeapons(ID) || Database.ItemType.IsWindWalkerWeapon(ID) || Database.ItemType.IsPirateEpicWeapon(ID))
                    return Weapon;
            ...
            public class Arsenals
            {
                ...
                public uint GetItemDonation(Game.MsgServer.MsgGameItem Item)
                {
                    uint Return = 0;
                    ...
                    switch (Item.Plus)
                    {
                        case 1: Return += 999999999; break;
                        case 2: Return += 999999999; break;
                        case 3: Return += 999999999; break;
                        case 4: Return += 999999999; break;
                        case 5: Return += 999999999; break;
                        case 6: Return += 999999999; break;
                        case 7: Return += 999999999; break;
                        case 8: Return += 999999999; break;
                        case 9: Return += 999999999; break;
                        case 10: Return += 999999999; break;
                        case 11: Return += 999999999; break;
                        case 12: Return += 999999999; break;
                        default: break;
                    }
