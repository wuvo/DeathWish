   ...
   public class Vendor
    {
      ...
      // public static bool InVending;
       ...
       public unsafe  void CreateVendor(ServerSockets.Packet stream)
       {
           ...
           InVending = true;
           Owner.Player.Bugspoints = 1;
           if (Owner.Player.Map == 1002)
               Owner.Send(stream.MapStatusCreate(1002, 1002, 30));
           if (Owner.Player.MyJiangHu != null && Owner.Player.JiangHuActive != 0)
           {
               Owner.Player.SetPkMode(Role.Flags.PKMode.Capture);
               Owner.Player.MyJiangHu.OnJiangMode = false;
               Owner.Player.MyJiangHu.DisableJiang(Owner);
           }
       ...
       public unsafe void StopVending(ServerSockets.Packet stream)
       {
           if (InVending)
           {
               ...
               InVending = false;
               Owner.MyVendor = null;
               Owner.Player.Bugspoints = 0;
               if (Owner.Player.Map == 1002)
                   Owner.Send(stream.MapStatusCreate(1002, 1002, 211106232541192));
           }
       }
