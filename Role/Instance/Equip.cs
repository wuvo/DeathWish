...
namespace DeathWish.Role.Instance
{
    public class Equip
    {
        ...
        public bool CanUpdatePerfectionItem(MsgGameItem item)
        {
            ...
            if (Database.ItemType.IsShield(LeftWeapon) || Database.ItemType.IsHossu(LeftWeapon))
                ItemsC = 1;
            if (count == 11 + ItemsC)
                return true;
            uint bigLevel = 0;
        ...
        public bool UsePirateEpicWeapon = false;
        ...
        public uint AleternanteLeftWeapon = 0;
        public uint AleternanteRightWeapon = 0;
        ...
        public bool SuperFuryGem = false;
        ...
        public int AttackSpeed(int MS_Delay)
        {
            ...
            //if (MS_Delay < 500)
            //    MS_Delay = 500;
            //if (Owner.Player.UID == 1029313 || Owner.Player.UID == 1029151)
            //    MS_Delay = 500;
            return MS_Delay;
        }
        ...
        public static void EquipBLade(ServerSockets.Packet stream, GameClient client, uint ID, Role.Flags.ConquerItem position, byte plus = 0, byte bless = 0, byte Enchant = 0
   , Role.Flags.Gem sockone = Flags.Gem.NoSocket
    , Role.Flags.Gem socktwo = Flags.Gem.NoSocket, bool bound = false, Role.Flags.ItemEffect Effect = Flags.ItemEffect.None)
        {
            Database.ItemType.DBItem DbItem;
        ...
        public static void CheakUp51(GameClient client, Game.MsgServer.MsgGameItem ItemDat)
        {
        ...
        public bool TryGetCurentValue(uint UID, out Game.MsgServer.MsgGameItem itemdata)
        {
            itemdata = null;
            foreach (var item in CurentEquip)
            {
        ...
        public Game.MsgServer.MsgGameItem GetItem(uint UID)
        {
            Game.MsgServer.MsgGameItem Item = null;
            ClientItems.TryGetValue(UID, out Item);
            return Item;
        }
        ...
        public bool Remove(Role.Flags.ConquerItem position, ServerSockets.Packet stream)
        {
            if (!FreeEquip(position))
            {
                ...
                if (position == Flags.ConquerItem.Steed || position == Flags.ConquerItem.SteedMount)
                {
                    if (Owner.Player.ContainFlag(MsgUpdate.Flags.Ride))
                    {
        ...
        {
            Game.MsgServer.MsgGameItem item = null;
            item = ClientItems.Values.Where(p => p.Position == (ushort)position).FirstOrDefault();
            return item;
        }
        ...
        public unsafe void UpdateStats(Game.MsgServer.MsgGameItem[] MyGear, ServerSockets.Packet stream)//loaditemstate
        {
            try
            {
                ...
                foreach (var item in MyGear)
                {
                    ...
                    if (Database.ItemType.ItemPosition(item.ITEM_ID) == (ushort)Role.Flags.ConquerItem.Head
                        ...
                        HeadID = item.ITEM_ID;
                    uint addlvl = item.PerfectionLevel;
                    //Owner.CalculateLeftWeapon = true;
                    //aici sa fac schimbare de position !!!!!!!!!!! // !FreeEquip(Flags.ConquerItem.RightWeapon) // Database.ItemType.IsTwoHand(

                    if ((Role.Flags.ConquerItem)(item.Position) == Flags.ConquerItem.RightWeapon || (Role.Flags.ConquerItem)(item.Position) == Flags.ConquerItem.AleternanteRightWeapon)
                        if (Database.ItemType.Is2Hand(item.ITEM_ID))
                        {
                            if (FreeEquip((Role.Flags.ConquerItem)(item.Position + 1)))
                            {
                                addlvl *= 2;
                            }
                        }
                    ...
                    if (Alternante)// kda stuff bdela
                    {
                        MsgGameItem Contrast;
                        if (TryGetEquip((Role.Flags.ConquerItem)(item.Position - 20), out Contrast))
                        {
                            addlvl = Math.Max(item.PerfectionLevel, Contrast.PerfectionLevel);  //  || (Role.Flags.ConquerItem)(Contrast.Position) == Flags.ConquerItem.AleternanteRightWeapon
                            if ((Role.Flags.ConquerItem)(Contrast.Position) == Flags.ConquerItem.RightWeapon)
                            {
                                if (Database.ItemType.Is2Hand(Contrast.ITEM_ID) && Database.ItemType.Is2Hand(item.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(Contrast.Position + 1)) && FreeEquip((Role.Flags.ConquerItem)(item.Position + 1)))
                                {
                                    addlvl = Math.Max(item.PerfectionLevel * 2, Contrast.PerfectionLevel * 2);
                                }
                                else if (Database.ItemType.Is2Hand(Contrast.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(Contrast.Position + 1)))
                                {
                                    uint leftlevel = 0;
                                    MsgGameItem currentleft;
                                    if (TryGetEquip((Role.Flags.ConquerItem)(item.Position + 1), out currentleft))
                                        leftlevel = currentleft.PerfectionLevel;
                                    addlvl = Math.Max(item.PerfectionLevel + leftlevel, Contrast.PerfectionLevel * 2);
                                    if (item.PerfectionLevel + leftlevel > Contrast.PerfectionLevel * 2)
                                        addlvl = item.PerfectionLevel;
                                }
                                else if (Database.ItemType.Is2Hand(item.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(item.Position + 1)))
                                {
                                    uint leftlevel = 0;
                                    MsgGameItem currentleft;
                                    if (TryGetEquip((Role.Flags.ConquerItem)(Contrast.Position + 1), out currentleft))
                                        leftlevel = currentleft.PerfectionLevel;
                                    addlvl = Math.Max(Contrast.PerfectionLevel + leftlevel, item.PerfectionLevel * 2);
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            addlvl = Math.Max(item.PerfectionLevel, Contrast.PerfectionLevel);  //  || (Role.Flags.ConquerItem)(Contrast.Position) == Flags.ConquerItem.AleternanteRightWeapon
                            if ((Role.Flags.ConquerItem)(Contrast.Position) == Flags.ConquerItem.AleternanteRightWeapon)
                            {
                                if (Database.ItemType.Is2Hand(Contrast.ITEM_ID) && Database.ItemType.Is2Hand(item.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(Contrast.Position + 1)) && FreeEquip((Role.Flags.ConquerItem)(item.Position + 1)))
                                {
                                    addlvl = Math.Max(item.PerfectionLevel * 2, Contrast.PerfectionLevel * 2);
                    ...
                    if (((Role.Flags.ConquerItem)(item.Position) == Flags.ConquerItem.LeftWeapon))
                    {
                        MsgGameItem Contrast;
                        if (TryGetEquip((Role.Flags.ConquerItem)(item.Position + 19), out Contrast))
                        {
                            if (Database.ItemType.Is2Hand(Contrast.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(Contrast.Position + 1)))
                            {
                                uint rightlevel = 0;
                                MsgGameItem currentright;
                                if (TryGetEquip((Role.Flags.ConquerItem)(item.Position - 1), out currentright))
                                    rightlevel = currentright.PerfectionLevel;
                                if (Contrast.PerfectionLevel * 2 >= item.PerfectionLevel + rightlevel)
                                    addlvl = 0;
                            }
                        }
                    }
                    else if ((Role.Flags.ConquerItem)(item.Position) == Flags.ConquerItem.AleternanteLeftWeapon)
                    {
                        MsgGameItem Contrast;
                        if (TryGetEquip((Role.Flags.ConquerItem)(item.Position - 21), out Contrast))
                        {
                            if (Database.ItemType.Is2Hand(Contrast.ITEM_ID) && FreeEquip((Role.Flags.ConquerItem)(Contrast.Position + 1)))
                            {
                                uint rightlevel = 0;
                    ...
                    try
                    {
                        ...
                        if (item.SocketOne != Role.Flags.Gem.NoSocket && item.SocketOne != Role.Flags.Gem.EmptySocket)
                        {
                            ...
                            if (item.SocketOne == Role.Flags.Gem.SuperKylinGem)
                                SuperKylinGem = true;
                            if (item.SocketOne == Role.Flags.Gem.SuperFuryGem)
                                SuperFuryGem = true;
                        }
                ...
                MsgGameItem Bottle;
                if (TryGetEquip(Role.Flags.ConquerItem.Bottle, out Bottle))
                {//فيه ملف فى ini فيه كل الكلام ده 
                    switch (Bottle.ITEM_ID)
                    {
                        ...
                        case 2100085:
                            {
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 5;
                                if (Owner.PrestigeLevel >= 300)
                                    Owner.Status.Defence += 300;
                                if (Owner.PrestigeLevel >= 400)
                                    Owner.PerfectionStatus.FreeSoul += 5;
                                if (Owner.PrestigeLevel >= 500)
                                    Owner.PerfectionStatus.KillingFlash += 5;
                                if (Owner.PrestigeLevel >= 600)
                                    Owner.PerfectionStatus.StraightLife += 5;
                                if (Owner.PrestigeLevel >= 648)
                                {
                        ...
                        case 2100095:
                            {
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 7;
                                if (Owner.PrestigeLevel >= 300)
                                    Owner.Status.Defence += 300;
                                if (Owner.PrestigeLevel >= 400)
                                    Owner.PerfectionStatus.FreeSoul += 7;
                                if (Owner.PrestigeLevel >= 500)
                                    Owner.PerfectionStatus.KillingFlash += 7;
                                if (Owner.PrestigeLevel >= 600)
                                    Owner.PerfectionStatus.StraightLife += 3;
                                if (Owner.PrestigeLevel >= 648)
                                {
                        ...
                        case 2100175:
                            {
                                if (Owner.PrestigeLevel >= 50)
                                    Owner.PerfectionStatus.ToxinEraser += 3;
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 1;
                                if (Owner.PrestigeLevel >= 150)
                                    Owner.PerfectionStatus.KillingFlash += 1;
                                break;
                            }
                        case 2100185:
                            {
                                if (Owner.PrestigeLevel >= 50)
                                    Owner.PerfectionStatus.ToxinEraser += 3;
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 2;
                                if (Owner.PrestigeLevel >= 200)
                                    Owner.PerfectionStatus.KillingFlash += 2;
                                if (Owner.PrestigeLevel >= 300)
                                    Owner.PerfectionStatus.FreeSoul += 2;
                                break;
                            }
                        case 2100195:
                            {
                                if (Owner.PrestigeLevel >= 50)
                                    Owner.PerfectionStatus.ToxinEraser += 3;
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 3;
                                if (Owner.PrestigeLevel >= 200)
                                    Owner.PerfectionStatus.BloodSpawn += 1;
                                if (Owner.PrestigeLevel >= 300)
                                    Owner.PerfectionStatus.FreeSoul += 3;
                                if (Owner.PrestigeLevel >= 500)
                                    Owner.PerfectionStatus.KillingFlash += 3;
                                if (Owner.PrestigeLevel >= 600)
                                    Owner.PerfectionStatus.StraightLife += 2;
                                break;
                            }
                        case 2100205:
                            {
                                if (Owner.PrestigeLevel >= 50)
                                    Owner.PerfectionStatus.ToxinEraser += 3;
                                if (Owner.PrestigeLevel >= 100)
                                    Owner.PerfectionStatus.DrainingTouch += 4;
                                if (Owner.PrestigeLevel >= 200)
                                    Owner.PerfectionStatus.BloodSpawn += 2;
                                if (Owner.PrestigeLevel >= 300)
                                    Owner.PerfectionStatus.FreeSoul += 4;
                                if (Owner.PrestigeLevel >= 500)
                                    Owner.PerfectionStatus.KillingFlash += 4;
                                if (Owner.PrestigeLevel >= 600)
                                    Owner.PerfectionStatus.StraightLife += 3;
                                if (Owner.PrestigeLevel >= 648)
                                    Owner.Status.MaxHitpoints += 300;
                ...
                try
                {
                    ...
                    if (Alternante)
                    {
                        if (!FreeEquip(Flags.ConquerItem.RightWeapon) && !FreeEquip(Flags.ConquerItem.LeftWeapon))
                        {
                            ...
                            if (Database.ItemType.IsWindWalkerWeapon(_left.ITEM_ID) && Database.ItemType.IsWindWalkerWeapon(_right.ITEM_ID))
                            {
                                MsgGameItem WindWe;
                                if (TryGetEquip(Flags.ConquerItem.AleternanteRightWeapon, out WindWe) && FreeEquip(Flags.ConquerItem.AleternanteLeftWeapon))
                                {
                                    foreach (var it in AllItems)
                                        if (it.Position > 20)
                                        {
